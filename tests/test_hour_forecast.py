...
def test_hour_is_separate_frozen_and_only_scores_revealed_minutes(tmp_path):
    raw,schedule,day=fixture(); history=aggregate(raw,schedule)
    ...
    class Model:
        ...
        def forecast_paths(self,window,targets,sample_count,seed):
            calls.append(window.copy());return np.tile([100,102,98,100.5],(sample_count,12,1)),.01
    ...
    partial=public(row,day.iloc[:150]);assert partial['outcome']['status']=='pending'
    assert partial['outcome']['actual']['close'] is None
    assert all(v is None for v in partial['outcome']['errors'].values())
    final=public(row,day.iloc[:180]);assert final['outcome']['status']=='complete'
    assert final['outcome']['observed_minutes']==60 and final['outcome']['errors']==dict(high=1.,low=1.,close=.5)
    assert final['outcome']['flat_close_error']==0
    assert public(row,day.iloc[:180].drop(index=140))['outcome']['status']=='incomplete'
    assert json.dumps(lab.hour_store.all()[0])==original
    save_outcomes(dict(clock=day.iloc[179].bar_end.isoformat(),hour_forecasts=[final]),tmp_path)
